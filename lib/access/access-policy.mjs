...
export const ACCESS_GROUPS = Object.freeze({
  ...
  team_leaders_ep: { code: "team_leaders_ep", name: "Team Leaders EP", description: "Herdam EPs e ganham Performance do EP." },
  ...
  product: { code: "product", name: "Produto", description: "Herdam Líderes e acessam Relatórios (Análises internas)." },
});
...
export const PAGE_ACCESS_METADATA = Object.freeze({
  ...
  statistical_crosses: Object.freeze({
    legacy: false,
    ownerOnly: false,
    preload: true,
    badge: null,
  }),
  internal_mechanisms_satisfaction: Object.freeze({
    legacy: false,
    ownerOnly: true,
    preload: false,
    badge: "Análise interna",
  }),
  internal_mechanisms_renewal_projection: Object.freeze({
    legacy: false,
    ownerOnly: true,
    preload: false,
    badge: "Análise interna",
...
const TEAM_LEADER_EXTRA_PAGES = Object.freeze(["ep_performance"]);

/** Páginas que Líderes/Produto (herança) não recebem pelo grant amplo de leaders. */
const LEADER_GRANT_EXCLUDED_PAGE_IDS = Object.freeze(["reports"]);
...
export const PAGE_ACCESS_REGISTRY = Object.freeze({
  ...
  internal_mechanisms_satisfaction: Object.freeze(["owner"]),
  internal_mechanisms_renewal_projection: Object.freeze(["owner"]),
...
export const API_PAGE_MAP = Object.freeze({
  ...
  "/api/internal-mechanisms-renewal-projection": "internal_mechanisms_renewal_projection",
...
export function pagesGrantedByGroups(groupCodes = []) {
  ...
  const pages = new Set();
  const leaderExcluded = new Set(LEADER_GRANT_EXCLUDED_PAGE_IDS);
  if (expanded.includes("leaders")) {
    for (const page of PAGES) {
      if (page.id === ACCESS_MANAGEMENT_PAGE_ID || leaderExcluded.has(page.id)) continue;
      pages.add(page.id);
    }
  }
  if (expanded.includes("product")) {
    pages.add("reports");
  }
 ...
 * @param {string} pageId
 */
export function canMutateReports(access) {
  return Boolean(access?.isActive !== false && access?.isOwner);
}

export function getPageForbiddenMessage(pageId) {
  if (pageId === "reports") return "Você não possui acesso aos relatórios.";
