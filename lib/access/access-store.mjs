...
function throwClassified(status, payload) {
  ...
  const httpStatus = classified.status || (status === 404 ? 503 : status || 503);
  const error = restError(httpStatus, classified.error, classified.code);
...
export async function fetchAccessUsers({ accessToken } = {}) {
  ...
  return asUserList(payload).map(mapUserRow);
}

export async function saveAccessRecord(input, { accessToken } = {}) {
  const payload = await analyticsFetch("/rest/v1/rpc/upsert_dashboard_access", {
    accessToken,
    write: true,
    method: "POST",
    profile: "public",
...
export async function deleteAccessRecord(input, { accessToken } = {}) {
  return analyticsFetch("/rest/v1/rpc/delete_dashboard_access", {
    accessToken,
    write: true,
    method: "POST",
    profile: "public",
