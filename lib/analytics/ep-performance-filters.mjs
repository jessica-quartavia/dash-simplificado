 ...
 * Sem filtro global de período; busca recorta por nome do EP.
 */
...
function clientLevelFilters(filters) {
  return filters.segment !== "all" || filters.program !== "all";
}
...
export function filterEpClients(clients, filters = {}) {
  const f = { ...defaultEpPerformanceFilters(), ...filters };
...
export function filterEpEngineers(engineers, filters = {}) {
  const f = { ...defaultEpPerformanceFilters(), ...filters };
  const needClientFilter = clientLevelFilters(f);
