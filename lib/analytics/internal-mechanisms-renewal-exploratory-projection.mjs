...
export function buildExploratoryRenewalProjection(portfolioActive, horizonClients, catalog, options = {}) {
  ...
  const top3 = buildTopMechanismsAssociative(train, horizonScored, horizonClients, catalog, options.minMechanismSample || 10);
  ...
  return {
    ...
    methodology: {
      name: "Regressão logística (features explicáveis) + estratificação programa × mecanismos como fallback",
      target: "renewed_binary (clients.ciclo > 1) entre clientes com ciclo válido",
      validation: "Holdout 20% determinístico por client_id",
      features: [
        "total_implemented_mechanisms",
        "has_mechanism",
        "tenure_days",
        "programa (Pharus/Davos)",
        "segmento (PRINCIPAL)",
      ],
      leakageNote: "Somente sinais disponíveis no snapshot atual; mecanismos atuais — viés de permanência.",
    },
    model: {
      trainN: train.length,
