...
/**
 * População do gráfico de aquisição — alinhada à V1:
 * - respeita o filtro de status (ex.: Ativos → adquiridos que permanecem ativos hoje);
 * - demais recortes cadastrais (EP, programa, segmento, busca, bandas) aplicados;
 * - período global só com `applyPeriod: true` (V1 não tinha; Resumo Executivo pode optar).
 *
 * `ignoreStatus: true` — uso exclusivo Resumo Executivo (evento histórico bruto).
 */
export function filterGeneralAcquisitionRows(clients, filters = {}, options = {}) {
  const rows = filterGeneralClients(clients, filters, {
    ...options,
    ignoreStatus: options.ignoreStatus === true,
  });
  if (options.applyPeriod !== true) return rows;
...
/**
 * Situação da carteira / clientes por status: ignora filtro de status (Somente ativos).
 * Demais recortes cadastrais continuam aplicados.
 */
export function filterGeneralPortfolioRows(clients, filters = {}, options = {}) {
  return filterGeneralClients(clients, filters, { ...options, ignoreStatus: true });
}
