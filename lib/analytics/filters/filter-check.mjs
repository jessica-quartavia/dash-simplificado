...
export function runPageBehavior(pageId) {
  ...
  checks.push(pass("clear_filters", cleared.search === "" && (cleared.period == null || cleared.period === "all") && !cleared.from && !cleared.to));
  checks.push(pass("period_ui_expected", pageShowsPeriodUi(pageId) === ["meetings", "financial_updates", "cancellations"].includes(pageId), pageShowsPeriodUi(pageId) ? "com período" : "sem período"));
  return checks;
}
