...
const RENEWAL_FIXTURE = [
  { clientId: "1", clientName: "Ana Souza", clientCode: "A1", analyticalStatus: "Ativo", engineer: "Nícolas Alves", segment: "PRIVATE", program: "Pharus", currentCycle: 3, renewalCount: 2, renewed: true, cycleValid: true },
  { clientId: "2", clientName: "João Lima", clientCode: "B2", analyticalStatus: "Ativo", engineer: "Outro EP", segment: "APEX", program: "Davos", currentCycle: 1, renewalCount: 0, renewed: false, cycleValid: true },
];
...
function checkPeriodUiPages() {
  return [
    ...
    pass("statistical_period_ui", pageShowsPeriodUi("statistical_crosses")),
  ];
}
...
export function runPageBehavior(pageId) {
  ...
  if (pageId === "general") {
    ...
    const acqPeriod = filterGeneralAcquisitionRows(
      GENERAL_FIXTURE,
      { ...defaultGeneralFilters(), period: "last_6m" },
    ...
    checks.push(pass("period_on_acquisition", acqPeriod.length < acqAll.length && acqPeriod.every((row) => (row.acquisitionDate || row.contractDate) >= "2026-02-19")));
  ...
  if (pageId === "renewal") {
    ...
    const renewedYes = filterRenewalClients(RENEWAL_FIXTURE, { ...defaultRenewalFilters(), renewed: "yes" });
    const activeOnly = filterRenewalClients(RENEWAL_FIXTURE, { ...defaultRenewalFilters(), status: "active" });
    ...
    checks.push(pass("status_filter", contract.uiFilters.includes("status")));
    checks.push(pass("renewed_yes_cycle", renewedYes.every((r) => r.renewed === true)));
    checks.push(pass("status_active_filter", activeOnly.every((r) => r.analyticalStatus === "Ativo")));
    checks.push(pass("not_period_ui", !pageShowsPeriodUi(pageId)));
    ...
    const EP_FIXTURE = [
      {
        ...
        clients: [
          { analyticalStatus: "Ativo", segment: "PRIVATE", program: "Pharus", totalMeetings: 2, meetingDates: ["2026-01-01"] },
          { analyticalStatus: "Congelado", segment: "PRIVATE", program: "Pharus", totalMeetings: 0, meetingDates: [] },
        ],
      },
      { engineer: "Outro EP", totalClients: 5, activeClients: 4, meetingCoverage: 40, cancelledShareOfPortfolio: 20, clients: [] },
    ];
    const filtered = filterEpEngineers(EP_FIXTURE, { ...defaultEpPerformanceFilters(), search: "Nícolas" });
    const statusFiltered = filterEpEngineers(EP_FIXTURE, { ...defaultEpPerformanceFilters(), status: "active" });
    checks.push(pass("period_ui", pageShowsPeriodUi(pageId)));
    checks.push(pass("status_filter_ui", contract.uiFilters.includes("status")));
    checks.push(pass("program_filter_ui", contract.uiFilters.includes("program")));
    checks.push(pass("no_fonte_ui", !contract.uiFilters.includes("fonte") && !contract.uiFilters.includes("source")));
    checks.push(pass("filter_engineer_search", filtered.length === 1 && filtered[0].engineer === "Nícolas Alves"));
    checks.push(pass("status_active_filter", statusFiltered[0]?.activeClients === 1));
  ...
  if (pageId === "temporal_indicators") {
    ...
    checks.push(pass("no_source_ui", !contract.uiFilters.includes("source")));
  ...
  if (pageId === "statistical_crosses") {
    ...
    checks.push(pass("period_ui", pageShowsPeriodUi(pageId)));
    checks.push(pass("search_param", params.get("search") === "Ana"));
    checks.push(pass("program_param", params.get("program") === "Pharus"));
    checks.push(pass("hire_from_param", Boolean(params.get("hireFrom"))));
    ...
    checks.push(pass("unimplemented_visual_documented", (contract.unimplementedVisualFilters || []).includes("matrixVars")));
    checks.push(pass("no_fonte_filter", !contract.uiFilters.includes("source") && !contract.uiFilters.includes("fonte")));
  }
  ...
  checks.push(pass("clear_filters", (() => {
    if (pageId === "statistical_crosses") {
      ...
      return c.program === "all" && c.engineer === "all" && c.status === "active_cancelled" && c.period === "all" && c.search === "" && c.minCoverage === 30 && c.minSample === 5;
    }
    return cleared.search === "" && (cleared.period == null || cleared.period === "all") && !cleared.from && !cleared.to;
  })()));
  checks.push(pass("period_ui_expected", pageShowsPeriodUi(pageId) === ["meetings", "financial_updates", "cancellations", "support", "ep_performance", "statistical_crosses"].includes(pageId), pageShowsPeriodUi(pageId) ? "com período" : "sem período"));
  return checks;
}
...
function checkReportsBehavior() {
  ...
  const storeSource = readFileSync(resolve(root, "lib/analytics/reports-store.mjs"), "utf8");
  ...
  const fixture = [
    {
      ...
      description: "Principais sinais pré-cancelamento",
  ...
  return [
    ...
    pass("reports_metadata_rpc", storeSource.includes("list_published_reports")),
    pass("reports_metadata_table", sql.includes("analytics.reports")),
    pass("reports_signed_open", reportsJs.includes("/api/analytics/reports") && reportsJs.includes("open=") && handlerJs.includes("createSignedUrl")),
...
function checkUxAdjustments() {
  ...
  return [
    ...
    pass("ep_period_ui", pageShowsPeriodUi("ep_performance")),
    pass("temporal_no_period_ui", !pageShowsPeriodUi("temporal_indicators")),
    pass("statistical_has_period_ui", pageShowsPeriodUi("statistical_crosses")),
