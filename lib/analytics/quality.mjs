...
import { scanFieldCoverage, countSelectColumn } from "./quality-coverage.mjs";
...
function loadQualityAuditClients() {
  return fetchAllRows("clients", "id,status,data_inicio_ciclo,created_at,data_churn");
}
...
export async function computeQualityPayload(options = {}) {
  ...
  const { items: scannedItems, loadErrors } = await scanFieldCoverage(FIELDS, {
    optionalTables: OPTIONAL_QUALITY_TABLES,
  });
