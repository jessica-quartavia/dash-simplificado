...
import { fetchAllRows as fetchTableRows } from "../data/supabase-rest.mjs";
import {
  getAnalyticsDataContext,
  perfDebugFromOptions,
  runWithAnalyticsDataContext,
} from "./analytics-data-context.mjs";
import { scanFieldCoverage, countSelectColumn, preloadQualityFieldRows } from "./quality-coverage.mjs";
...
async function fetchAllRows(table, select, order = "id.asc") {
  return fetchTableRows({ table, select, order: order || "id.asc" });
}
...
function preEntryMeetingsAudit(clients, calendly, manual) {
...
function closedWithoutCancellationAudit(clients, cancellations) {
  const {
    map: cancelMap,
    multiples,
    rowsWithoutClientId,
...
export async function computeQualityCore(options = {}) {
  const scanStarted = Date.now();
  const optionalTables = options.optionalTables || OPTIONAL_QUALITY_TABLES;
  const { rowsByTable, loadErrors } = await preloadQualityFieldRows(FIELDS, { optionalTables });
  const { items: scannedItems } = await scanFieldCoverage(FIELDS, {
    optionalTables,
    rowsByTable,
    loadErrors,
  });
  const scanMs = Date.now() - scanStarted;
  const clients = rowsByTable.get("clients") || [];
  const cancellations = rowsByTable.get("cancellations") || [];
  const calendly = rowsByTable.get("client_meetings") || [];
  const manual = rowsByTable.get("manual_meetings") || [];
  ...
  return {
    ...
    scanMs,
    audits: {
      statusConsistency: clientsStatusConsistency(clients),
      stayAudit: closedWithoutCancellationAudit(clients, cancellations),
      preEntryAudit: preEntryMeetingsAudit(clients, calendly, manual),
    },
  };
}

export async function computeQualityPayload(options = {}) {
  ...
  try {
    ...
    throw new Error(error instanceof Error ? error.message : "Configuração inválida");
  }

  const scanStarted = Date.now();
  const [core, supportResult, pharusResult] = await Promise.all([
    computeQualityCore(),
    computeSupportPayload().then((value) => ({ ok: true, value })).catch((error) => ({ ok: false, error })),
    computePharusMechanismsPayload().then((value) => ({ ok: true, value })).catch((error) => ({ ok: false, error })),
  ]);
  const scanMs = Date.now() - scanStarted;
  const { scannedItems, loadErrors, audits } = core;

  const data = scannedItems.map(({ domain, table, column, totalRows, missingRows }) => {
    const key = `${table}.${column}`;
  ...
  const warnings = [];
  const hardErrors = [];
  for (const [table, error] of loadErrors.entries()) {
    const warning = buildFieldWarning(error, { table, column: null });
    warnings.push(warning);
  }

  try {
    if (supportResult.ok) {
      data.push(...buildAcionamentosQualityFromSupport(supportResult.value));
    } else {
  ...
  try {
    const pharusPayload = pharusResult.ok ? pharusResult.value : null;
    if (!pharusResult.ok) throw pharusResult.error;
  ...
  try {
    const consistency = audits.statusConsistency;
  ...
  try {
    const stayAudit = audits.stayAudit;
    const statusField = data.find((item) => item.table === "clients" && item.column === "status");
    if (statusField) {
  ...
  try {
    const preEntryAudit = audits.preEntryAudit;
    for (const key of [
  ...
  const payload = {
    ...
    performance: { scanMs, method: "table_scan" },
  };
  ...
  return payload;
  }, { perfDebug: perfDebugFromOptions(options), page: "quality" });
}
