...
export const EXECUTIVE_METRIC_REGISTRY = Object.freeze([
  ...
  { id: "top_cancellation_reasons", label: "Principais motivos de cancelamento", section: "clientHealth", sourcePage: "cancellations", sourceMetricId: "cancellation_reasons", visualization: "rank-list", filters: FILTER_STOCK, limitations: "Top 3 categorizados; exclui não categorizados." },
  ...
  { id: "total_renewals", label: "Quantidade de renovações", section: "clientHealth", sourcePage: "renewal", sourceMetricId: "total_renewals", visualization: "kpi", filters: { program: true, engineer: true, segment: true, status: false }, limitations: "sum(max(ciclo-1,0)) sobre elegíveis." },
  { id: "renewal_eligible_clients", label: "Clientes aptos para renovação", section: "clientHealth", sourcePage: "renewal", sourceMetricId: "renewal_eligible_clients", visualization: "pending", filters: FILTER_STOCK, limitations: RENEWAL_ELIGIBLE_RULE_STATUS.message },
  { id: "renewals_per_active_client", label: "Renovações por cliente ativo", section: "clientHealth", sourcePage: "derived", sourceMetricId: "renewals_per_active_client", visualization: "kpi", filters: FILTER_STOCK, limitations: "totalRenewals ÷ activeClients (derivado transparente)." },

  { id: "top_ep_renewed_share", label: "EP com maior % renovada", section: "ep", sourcePage: "ep_performance", sourceMetricId: "renewed_portfolio_percentage", visualization: "ep-card", filters: { program: true, engineer: true, segment: true, status: false }, limitations: "renewedPortfolioPercentage por EP." },
  { id: "top_ep_implementation_share", label: "EP com maior % implantada", section: "ep", sourcePage: "ep_performance", sourceMetricId: "implemented_share_by_ep", visualization: "ep-card", filters: { program: true, engineer: true, segment: true, status: false }, limitations: "clientsWithImplementedMechanisms ÷ totalClients do EP." },

  { id: "temporal_top_signals", label: "Sinais de cancelamento", section: "temporal", sourcePage: "temporal_indicators", sourceMetricId: "pre_cancellation_signals", visualization: "signal-list", filters: FILTER_PROGRAM_ONLY, limitations: "Sinais determinísticos; associação descritiva, sem causalidade." },
  { id: "temporal_signal_distribution", label: "Clientes por quantidade de sinais", section: "temporal", sourcePage: "temporal_indicators", sourceMetricId: "signal_count_distribution", visualization: "hbar", filters: FILTER_PROGRAM_ONLY, limitations: "Faixas 0/1/2/3/4+ sinais pré-cancelamento." },
]);
...
export function topMechanismDistribution(items = [], limit = 5) {
  const sorted = [...items].sort((a, b) => (b.count || b.clients || 0) - (a.count || a.clients || 0));
