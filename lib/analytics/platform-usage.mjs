...
export async function computePlatformUsagePayload() {
  ...
  const { eligible: eligibleEvents, stats: eventReconciliation } = reconcileLoginEvents(rawEvents, userDirectory, now);
  const excludedByDirectory = {
    corporate: eventReconciliation.excludedCorporateEmail,
    demo: eventReconciliation.excludedDemoEmail,
    unknownEmail: 0,
  };
  for (const event of eligibleEvents) {
    const userId = eventUserId(event);
    if (!userDirectory.get(userId)?.email) excludedByDirectory.unknownEmail += 1;
  }
