import { pharusRestFetchAll, probePharusAccess, pharusRestConfigurationError } from "../data/pharus-rest.mjs";
...
async function fetchPharusEvents() {
  return pharusRestFetchAll("events", PHARUS_EVENTS_SELECT, {
    schema: "metrics",
...
async function loadUserDirectory(warnings) {
  ...
  try {
    const personalRows = await pharusRestFetchAll(
      "personal_info",
      "user_id,name,alternative_email,phone",
      { schema: "core", maxRows: 200_000 },
    );
  ...
  try {
    const preRows = await pharusRestFetchAll("pre_registrations", "user_id,email,name", {
      schema: "core",
      maxRows: 200_000,
    });
...
export async function computePlatformUsagePayload() {
  const configError = pharusRestConfigurationError();
  if (configError) {
  ...
  const metricsProbe = probe.tables?.find((t) => t.schema === "metrics" && t.table === "events");
  if (!metricsProbe?.accessible) {
    warnings.push({
      code: "PHARUS_METRICS_UNAVAILABLE",
      label: "metrics.events indisponível",
      severity: "warning",
      message: metricsProbe?.message || "Não foi possível ler metrics.events com anon key.",
    });
    return unavailablePayload({ warnings, probe, message: "Fonte ainda não disponível para leitura." });
  }

  let pharusEvents = [];
  try {
    pharusEvents = await fetchPharusEvents();
  } catch (error) {
    warnings.push({
      code: "PHARUS_METRICS_FETCH",
      label: "metrics.events indisponível",
      severity: "warning",
