...
function normalizeClientProgram(value) {
  const text = fold(value);
  if (text.includes("davos")) return "Davos";
  if (text.includes("pharus")) return "Pharus";
  return null;
...
function npsByProgram(quarterNpsRows, clientMap, programName) {
  const scores = quarterNpsRows
    .filter((row) => {
      const client = clientMap.get(String(row.client_id || ""));
      return normalizeClientProgram(client?.programa) === programName;
    })
...
export function buildNpsProgramBreakdown(quarterNpsRows, clientMap) {
  ...
  for (const row of quarterNpsRows || []) {
    ...
    if (score == null) continue;
    const client = clientMap.get(String(row.client_id || ""));
    const program = normalizeClientProgram(client?.programa) || "Não informado";
