...
import { getAnalyticsDataContext, runWithAnalyticsDataContext } from "./analytics-data-context.mjs";
import { excludedClientIds, filterExcludedClients } from "./data-exclusions.mjs";
import { dedupeNpsResponses } from "./nps-metrics.mjs";
import { normalizeProgramFilter, resolveClientProgram } from "./filters/program.mjs";

async function fetchTable(table, select, order = "created_at.asc", extra = {}) {
  return fetchAllRows({ table, select, order: order || "created_at.asc", ...extra });
}

const SATISFACTION_NPS_SELECT =
  "id,typeform_response_id,typeform_form_id,client_id,client_name,client_email,tipo_de_forms,score,comment,raw_payload,created_at,submitted_at";
const SATISFACTION_CSAT_SELECT =
...
const USED_FIELDS = [
  ...
  { table: "nps_responses", column: "submitted_at", role: "npsSubmittedAt" },
...
function normalizeSatisfactionClientCode(value) {
  return String(value || "").trim().toUpperCase();
}

function officialClientName(client) {
  const name = String(client?.name ?? "").trim();
  return name || null;
}

function clientsByCodeFromMap(clientMap) {
  const byCode = new Map();
  for (const client of clientMap.values()) {
    const code = normalizeSatisfactionClientCode(client?.codigo);
    if (code && !byCode.has(code)) byCode.set(code, client);
  }
  return byCode;
}

function resolveOfficialSatisfactionClient(row, clientMap, clientsByCode) {
  const id = row?.clientId != null ? String(row.clientId).trim() : "";
  if (id && clientMap.has(id)) return clientMap.get(id);
  const code = normalizeSatisfactionClientCode(row?.clientCode || row?.codigo);
  if (code && clientsByCode.has(code)) return clientsByCode.get(code);
  return null;
}

export function enrichSatisfactionClientRows(clientRows, clientMap) {
  const map = clientMap instanceof Map ? clientMap : new Map();
  const clientsByCode = clientsByCodeFromMap(map);
  return (Array.isArray(clientRows) ? clientRows : []).map((row) => {
    const client = resolveOfficialSatisfactionClient(row, map, clientsByCode);
    const program = resolveClientProgram(client);
    if (
      typeof process !== "undefined"
      && process.env?.SATISFACTION_DEBUG === "1"
    ) {
      console.info("[satisfaction] name-resolve", {
        clientId: row?.clientId || null,
        clientCode: client?.codigo || row?.clientCode || null,
        resolvedClient: Boolean(client),
        clientNamePresent: Boolean(officialClientName(client)),
        epPresent: Boolean(client?.engenheiro_patrimonial),
      });
    }
    return {
      ...row,
      clientName: officialClientName(client) || "Não informado",
      clientCode: client?.codigo || null,
      engineer: client?.engenheiro_patrimonial || "Não informado",
      program: program === "Não informado" ? null : program,
      analyticalStatus: client?.status || null,
    };
  });
}

function isOfficialNpsCandidate(row) {
...
function npsCountsFromScores(scores) {
  const valid = (scores || []).filter((score) => score != null);
  return {
    n: valid.length,
    promoters: valid.filter((score) => score >= 9).length,
    neutrals: valid.filter((score) => score >= 7 && score <= 8).length,
    detractors: valid.filter((score) => score <= 6).length,
    nps: calcNps(valid),
  };
}

function breakdownFromProgramBuckets(buckets) {
  const build = (program, scores) => ({
    program,
    ...npsCountsFromScores(scores),
  });
  const pharus = build("Pharus", buckets.Pharus);
  const davos = build("Davos", buckets.Davos);
  const unknown = build("Não informado", buckets["Não informado"]);
  const allScores = [...buckets.Pharus, ...buckets.Davos, ...buckets["Não informado"]];
  return {
    total: build("Total", allScores),
    pharus,
    davos,
    unknown,
  };
...
function programBucketsFromDedupedRows(dedupedRows, clientMap) {
  const buckets = {
    Pharus: [],
    Davos: [],
    "Não informado": [],
  ...
  for (const row of dedupedRows || []) {
    ...
    const program = resolveClientProgram(client);
    const key = buckets[program] ? program : "Não informado";
    buckets[key].push(score);
  }
  return buckets;
}

/** NPS oficial — última resposta por cliente (nps-metrics), Programa só BASE QV. */
export function buildOfficialNpsProgramBreakdown(allNpsRows, clientMap) {
  const { rows: deduped, meta } = dedupeNpsResponses(allNpsRows);
  const breakdown = breakdownFromProgramBuckets(programBucketsFromDedupedRows(deduped, clientMap));
  return { breakdown, meta, dedupedRows: deduped };
}

/** NPS filtrado por Programa com a mesma regra oficial (headline quando filtro ativo). */
export function resolveFilteredOfficialNps(allNpsRows, clientMap, programFilter = "all") {
  const normalized = normalizeProgramFilter(programFilter);
  const { breakdown } = buildOfficialNpsProgramBreakdown(allNpsRows, clientMap);
  if (normalized === "all") return null;
  if (normalized === "Pharus") return breakdown.pharus;
  if (normalized === "Davos") return breakdown.davos;
  return breakdown.unknown;
}

/** Compara escopos A–E para diagnóstico (não escolhe regra pelo número). */
export function computeNpsScopeComparison(allNpsRows, clientMap, npsQuarterly = []) {
  const latestQuarter = resolveSatisfactionQuarter(npsQuarterly, "latest");
  const quarterRows = rowsInQuarter(allNpsRows, latestQuarter);
  const allValidScores = (allNpsRows || [])
    .filter(isOfficialNpsCandidate)
    .map((row) => npsScore(row.score));
  const quarterAllScores = quarterRows
    .filter(isOfficialNpsCandidate)
    .map((row) => npsScore(row.score));
  const quarterLatestScores = latestNpsPerClient(quarterRows)
    .map((row) => npsScore(row.score))
    .filter((score) => score != null);
  const { rows: deduped } = dedupeNpsResponses(allNpsRows);
  const officialScores = deduped.map((row) => row.score);
  const toScope = (scope, scores) => ({
    scope,
    ...npsCountsFromScores(scores),
  });
  return [
    toScope("A — todas respostas válidas históricas", allValidScores),
    toScope("B — última por cliente (histórico, submitted_at desc)", officialScores),
    toScope("C — todas respostas no trimestre mais recente", quarterAllScores),
    toScope("D — última por cliente no trimestre (V1 satisfação)", quarterLatestScores),
    toScope("E — oficial nps-metrics / relatório estatístico", officialScores),
  ];
}

export function npsClassificationDistribution(counts) {
  const total = counts?.n || 0;
  return [
    { label: "Promotores", count: counts?.promoters || 0, percent: pct(counts?.promoters || 0, total) },
    { label: "Neutros", count: counts?.neutrals || 0, percent: pct(counts?.neutrals || 0, total) },
    { label: "Detratores", count: counts?.detractors || 0, percent: pct(counts?.detractors || 0, total) },
  ];
}

function npsByProgram(quarterNpsRows, clientMap, programName) {
  const counts = npsCountsFromScores(
    quarterNpsRows
      .filter((row) => resolveClientProgram(clientMap.get(String(row.client_id || ""))) === programName)
      .map((row) => npsScore(row.score))
      .filter((score) => score != null),
  );
  return {
...
/** Breakdown NPS por Programa BASE QV (Pharus / Davos / Não informado). */
export function buildNpsProgramBreakdown(quarterNpsRows, clientMap) {
  const buckets = {
    Pharus: [],
    Davos: [],
    "Não informado": [],
  };
  for (const row of quarterNpsRows || []) {
    const score = npsScore(row.score);
    if (score == null) continue;
  ...
  return breakdownFromProgramBuckets(buckets);
}
...
export function buildSatisfactionQuarterView({
  ...
  return {
    ...
    summary: {
      ...
      npsPharus: npsByProgram(quarterNpsRows, clientMap, "Pharus"),
      npsDavos: npsByProgram(quarterNpsRows, clientMap, "Davos"),
      npsProgramBreakdown: buildNpsProgramBreakdown(quarterNpsRows, clientMap),
    },
    distributions: {
      npsClassification: [
...
export function buildSatisfactionPayload({
  ...
  const official = buildOfficialNpsProgramBreakdown(allNpsRows, clientMap);
  const officialTotal = official.breakdown.total;
  const scopeComparison = computeNpsScopeComparison(allNpsRows, clientMap, npsQuarterly);
  ...
  return {
    ...
    methodology: {
      npsRule:
        "NPS = (% promotores − % detratores). População: última resposta NPS válida por client_id (submitted_at desc, created_at desc), nota 0–10, tipo_de_forms NPS quando preenchido — mesma regra do relatório estatístico / EP (nps-metrics.mjs). Programa exclusivamente de BASE QV clients.",
      npsQuarterNote:
        "Recorte trimestral (created_at desc por cliente) permanece só para CSAT, tabela de clientes e série trimestral — não alimenta o card NPS principal.",
    ...
    population: {
      ...
      npsClients: officialTotal.n,
    },
    summary: {
      ...scoped.summary,
      nps: officialTotal.nps,
      promoters: officialTotal.promoters,
      neutrals: officialTotal.neutrals,
      detractors: officialTotal.detractors,
      npsDistinctClients: officialTotal.n,
      npsResponses: officialTotal.n,
      npsCoveragePercent: pct(officialTotal.n, totalClients),
      quarterScopedNps: scoped.summary.nps,
      quarterScopedRespondents: scoped.summary.npsDistinctClients,
    },
    benchmarks: {
      pharus: official.breakdown.pharus,
      davos: official.breakdown.davos,
      unknown: official.breakdown.unknown,
      total: official.breakdown.total,
    },
    officialNps: {
      meta: official.meta,
      scopeComparison,
    ...
    distributions: {
      ...
      npsClassification: npsClassificationDistribution(officialTotal),
      npsMonthly,
      npsQuarterly,
    },
    npsTransitions,
...
export async function computeSatisfactionPayload(options = {}) {
  ...
  const [clientsRaw, npsRowsRaw, csatRowsRaw, npsSends] = await Promise.all([
    fetchTable("clients", "id,codigo,name,email,status,engenheiro_patrimonial,programa,davos_contrato_assinado", "id.asc"),
    fetchTable("nps_responses", SATISFACTION_NPS_SELECT, "created_at.asc", { canonical: false }),
    fetchTable("csat_responses", SATISFACTION_CSAT_SELECT, "created_at.asc", { canonical: false }),
  ...
  const payload = buildSatisfactionPayload({ clients, clientMap, npsRowsRaw: npsRows, csatRowsRaw: csatRows, npsSends });
  payload._fetchStats = getAnalyticsDataContext()?.snapshot() || [];
  return payload;
  }, { page: "satisfaction", perfDebug: Boolean(options.perfDebug) });
}

export function toPublicSatisfactionPayload(payload) {
  if (!payload || typeof payload !== "object") return payload;
  const scopeInputs = payload.scopeInputs;
  if (!scopeInputs) return payload;
  return {
    ...payload,
    scopeInputs: {
      ...scopeInputs,
      allNpsRows: (scopeInputs.allNpsRows || []).map(slimScopeNpsRow),
      allCsatRows: (scopeInputs.allCsatRows || []).map(slimScopeCsatRow),
    },
  };
}

function slimScopeNpsRow(row) {
  return {
    id: row?.id,
