...
export function buildSatisfactionPayload({
  ...
  const latestClientNps = latestNpsPerClient(allNpsRows);
  const npsScores = latestClientNps.map((row) => npsScore(row.score)).filter((score) => score != null);
  ...
  const csatScores = allCsatRows.map((row) => csatScore(row.score)).filter((score) => score != null);
  const latestNpsRow = [...latestClientNps].sort((a, b) => parseDate(b.created_at) - parseDate(a.created_at))[0] || null;
  ...
  const clientRows = buildRows(latestClientNps, allCsatRows, clientPrograms);
  ...
  return {
    ...
    methodology: {
      npsRule:
        "Última resposta NPS válida por cliente (created_at desc), após dedupe por typeform_response_id/id — mesma regra da tela Pesquisa de Satisfação V1.",
      npsHelperNote:
        "O helper nps-metrics (EP/Análises) usa submitted_at desc; esta página mantém a regra consolidada da tela de satisfação.",
      csatRule: "Média de csat_responses.score com tipo_de_forms contendo CSAT; satisfeito = 5.",
      cesRule: "CES não possui fonte estruturada — indicador ausente do Dash Kids.",
    },
    population: {
      totalClients,
      respondents: feedbackClients.length,
      npsClients: latestClientNps.length,
    },
    summary: {
      nps: calcNps(npsScores),
      latestNps: latestNpsScore,
      latestNpsAt: latestNpsRow?.created_at || null,
      npsResponses: allNpsRows.length,
      npsDistinctClients: latestClientNps.length,
      npsSends: npsSends.length,
      promoters,
      neutrals,
      detractors,
      csatAverage: average(csatScores),
      csatResponses: allCsatRows.length,
      ...
      npsCoveragePercent: pct(latestClientNps.length, totalClients),
    },
    distributions: {
      npsClassification: [
      ...
      csatSatisfaction: [
        ...
        { label: "Não satisfeitos (1-4)", count: Math.max(0, csatScores.length - satisfiedCsat), percent: pct(Math.max(0, csatScores.length - satisfiedCsat), csatScores.length) },
      ],
      npsMonthly,
      npsQuarterly,
    },
