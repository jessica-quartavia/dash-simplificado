...
export async function handlePlatformUsageRequest(request, deps = {}) {
  ...
  try {
    ...
    console.info(`[platform-usage] status=200 total=${Date.now() - startedAt}ms status=${payload?.status ?? "—"} events=${payload?.summary?.eventsLoaded ?? payload?.summary?.appPharusEvents ?? 0} users=${payload?.summary?.totalUsers ?? "—"}`);
