...
export async function handleMeetingsRequest(request, deps = {}) {
  ...
  try {
    ...
    console.info(
      `[meetings] status=200 auth=${authMs}ms baseQv=${timing.baseQvMs ?? "—"}ms calendly=${timing.calendlyTypesMs ?? "—"}ms transform=${timing.transformMs ?? "—"}ms compute=${timing.totalMs ?? "—"}ms total=${Date.now() - startedAt}ms`,
    );
