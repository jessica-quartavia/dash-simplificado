...
import { countTotalMeetingsFromSource } from "./meeting-total-count.mjs";
...
export function applyMeetingFilters(clients, filters = {}, options = {}) {
  ...
  const mapped = filtered.map((c) => {
    ...
    const scopedTotal = c.totalCountSource
      ? countTotalMeetingsFromSource(c.totalCountSource, parseDate(c.totalCountSource.entryDate || c.entryDate), period, now).total
      : meetings.length;
    const enriched = enrichMeetingClientMetrics(c, meetings, {
      periodActive: true,
      periodDivisor: period.divisor,
      now,
    });
    return { ...enriched, totalMeetings: scopedTotal, _periodScoped: true };
  });
