...
const CLIENT_SELECT = "id,codigo,name,engenheiro_patrimonial,data_inicio_ciclo,created_at,programa,davos_contrato_assinado";
...
export function medianDays(values) {
  const nums = (values || []).filter((value) => value != null && Number.isFinite(value) && value >= 0).sort((a, b) => a - b);
  if (!nums.length) return null;
...
export function buildPatrimonialPlanSummary({
  ...
  return {
    ...
    value: medianDays(eligibleDays),
    meanValue: meanDays(eligibleDays),
    calculation: "median",
    coveragePercent: coverage.percent,
    coverage,
    clients: clientRows,
...
export function toPublicPatrimonialPlanPayload(payload) {
  ...
  return {
    ...
    approvalTime: {
      ...
      unit: "days",
      calculation: approval.calculation || "median",
      calculationLabel: approval.calculation === "mean" ? "média" : "mediana",
      meanValue: approval.meanValue ?? null,
