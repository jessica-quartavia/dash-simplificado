...
import { crosswalkFromIdentityMatch } from "../identity-match.mjs";
...
function buildNormalizedLink({
  ...
  return {
    ...
    baseQvLinkId,
    pharusLinkId,
    pharusUserId: pharusUserId || null,
  };
}
...
function mergeLink(existing, incoming) {
  ...
  return {
    ...
    pharusUserId: existing.pharusUserId || incoming.pharusUserId || null,
...
function rollupClientRow(clientId, links, baseRow = null) {
  ...
  return {
    ...
    davosContractSigned: baseRow?.davosContractSigned === true,
    clientProgram: baseRow?.clientProgram || resolveClientProgram(baseRow),
    ...
    mechanisms: links.map((link) => ({
      ...
      pharusUserId: link.pharusUserId || null,
    })),
  };
}
...
function buildQvClientsForIdentityMatch(baseClients, clientsRaw) {
  const rawById = new Map((clientsRaw || []).map((c) => [String(c.id), c]));
  return (baseClients || []).map((client) => {
    const raw = rawById.get(String(client.clientId)) || {};
    return {
      id: String(client.clientId),
      codigo: client.clientCode || raw.codigo || null,
      name: client.clientName || raw.name || null,
      email: raw.email || client.email || null,
      cpf: raw.cpf || raw.cpf_digits || null,
      cpf_digits: raw.cpf_digits || null,
      phone: raw.phone || raw.phone_digits || null,
      phone_digits: raw.phone_digits || null,
      linked_user_id: raw.linked_user_id || null,
      engineer: client.engineer || raw.engenheiro_patrimonial || null,
      mechanismCount: client.available || (client.mechanisms || []).length || 0,
    };
  });
}

function buildPharusUsersForIdentityMatch(pharusPayload, pharusProfiles) {
  ...
  return [...mechCountByUser.entries()].map(([userId, mechanismCount]) => {
...
export function consolidateMechanismsPayload({
  ...
  const crosswalk = pharusPayload
    ? crosswalkFromIdentityMatch(
        buildQvClientsForIdentityMatch(baseClients, clientsRaw),
        buildPharusUsersForIdentityMatch(pharusPayload, pharusProfiles),
      )
    : buildPharusClientCrosswalk(clientsRaw, pharusProfiles);
  ...
  const clientAudit = pharusPayload
    ...
    : {
        ...
        consolidationMode: "base_qv_only",
  ...
  for (const [userId, userLinks] of pharusLinksByUser.entries()) {
    ...
    for (const link of userLinks) {
      ...
      const normalized = { ...link, canonicalClientId, pharusUserId: userId };
  ...
  for (const [canonicalClientId, links] of clientsByCanonical.entries()) {
    ...
    if (!baseRow && canonicalClientId.startsWith("pharus:")) {
      ...
      const profile = pharusProfiles.get(userId) || {};
      const matchedQvId = crosswalk.byUserId.get(String(userId)) || null;
      const matchedBase = matchedQvId ? baseClientMap.get(String(matchedQvId)) : null;
      consolidatedClients.push(
        rollupClientRow(
          ...
          links,
          {
            ...
            engineer: matchedBase?.engineer || "Não informado",
            segment: matchedBase?.segment || "Dados insuficientes",
            analyticalStatus: matchedBase?.analyticalStatus || "Não informado",
            program: matchedBase?.program || null,
            davosContractSigned: matchedBase?.davosContractSigned === true,
            programUnknown: !matchedBase,
            pharusUserId: userId,
          },
        ),
      );
  ...
  const baseQvClientsImplemented = new Set();
  for (const client of baseClients) {
    if ((client.mechanisms || []).some((m) => m.status === "Implementado")) {
      baseQvClientsImplemented.add(String(client.clientId));
    }
  }
  const pharusUsersImplemented = new Set();
  for (const row of pharusPayload?.rows || []) {
    const uid = String(row.userId || row.user_id || "");
    if (!uid) continue;
    if (mapPharusStatus(row.status) === "Implementado") pharusUsersImplemented.add(uid);
  }
  const overlapImplementedPeople = new Set();
  for (const uid of pharusUsersImplemented) {
    const qvId = crosswalk.byUserId.get(String(uid));
    if (qvId && baseQvClientsImplemented.has(String(qvId))) overlapImplementedPeople.add(String(qvId));
  }

  const implementationAudit = {
    baseQvImplementedLinks: baseClients.reduce((sum, c) => sum + (c.implemented || 0), 0),
    baseQvClientsImplemented: baseQvClientsImplemented.size,
    pharusImplementedLinks: [...(pharusPayload?.rows || [])].filter((r) => mapPharusStatus(r.status) === "Implementado").length,
    pharusUsersImplemented: pharusUsersImplemented.size,
    overlapImplementedPeople: overlapImplementedPeople.size,
    consolidatedImplementedLinks: implementedLinks,
    consolidatedClientsImplemented: clientsWithImplementedMechanism,
