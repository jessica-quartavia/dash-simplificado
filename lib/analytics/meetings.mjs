...
import {
  ...
  buildTotalCountSourceForClient,
  countTotalMeetingsForClient,
} from "./meeting-total-count.mjs";
import {
  average,
  blankToNull,
...
export function buildMeetingsPayload({
  ...
  const totalBreakdownParts = [];

  for (const client of clients || []) {
    const clientId = String(client.id);
    const entry = resolveClientEntry(client);
    const entryDate = entry.date;
    const totalCount = countTotalMeetingsForClient({
      clientId,
      calendlyRows: calendlyRows || [],
      manualRows: manualRows || [],
      entryDate,
      period: null,
      now,
    });
    totalBreakdownParts.push(totalCount);
    const totalCountSource = {
      ...buildTotalCountSourceForClient(calendlyRows || [], manualRows || [], clientId),
    ...
    clientRows.push({
      ...
      totalMeetings: totalCount.total,
      totalCountSource,
      ...
      meetings: annotatedMeetings.map((m) => {
        ...
        return {
          ...
          externalUri: m.externalUri || null,
  ...
  const totalMeetingsBreakdown = aggregateTotalMeetingBreakdown(totalBreakdownParts);

  const summary = {
    totalMeetings: totalMeetingsBreakdown.total,
    totalMeetingsBreakdown,
    futureMeetings,
  ...
  return {
    ...
    metadata: {
      ...
      totalMeetingsRule:
        "client_meetings válidas + manual_meetings válidas sem correspondência (mesmo client_id e start_time). Reuniões anteriores à entrada excluídas.",
      totalMeetingsCardNote:
...
export async function computeMeetingsPayload(options = {}) {
  ...
  payload._auditSources = {
    calendlyRows: calendlyTimed.value.filter(keepClient),
    manualRows: manualTimed.value.filter(keepClient),
  };
  payload._fetchStats = getAnalyticsDataContext()?.snapshot() || [];
  return payload;
  }, { page: "meetings", perfDebug: Boolean(options.perfDebug) });
}
