...
export async function computeCancellationsPayload(options = {}) {
  ...
  const [bundle, statusRows] = await Promise.all([
    preloadExecutiveSharedBundle(),
    fetchTable("cancellation_statuses", STATUS_DIM_SELECT, "display_order.asc"),
  ]);
  const clients = bundle.clients;
  const cancellations = bundle.cancellations;
  const financialRows = bundle.financialRows;
  const calendlyRows = bundle.calendlyRows;
  const manualRows = bundle.manualRows;
  const attendanceRows = bundle.attendanceRows;
  ...
  return payload;
  }, { perfDebug: perfDebugFromOptions(options), page: "cancellations" });
}
...
export { buildPayload as buildCancellationsPayloadFromRaw };
