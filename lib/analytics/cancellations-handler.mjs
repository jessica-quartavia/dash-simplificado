/**
 * Handler HTTP de Cancelamento.
 */
import { requireCorporateAuth } from "../auth.mjs";
import { dataConfigurationError } from "../env.mjs";
import { computeCancellationsPayload, toPublicCancellationsPayload } from "./cancellations.mjs";
...
export async function handleCancellationsRequest(request, deps = {}) {
  ...
  if (denied) {
    console.info(`[cancellations] status=${denied.status} auth=${authMs}ms total=${Date.now() - startedAt}ms`);
    return denied;
  }
  ...
  try {
    ...
    const compute = deps.computeCancellationsPayload || computeCancellationsPayload;
    ...
    console.info(`[cancellations] status=200 auth=${authMs}ms total=${Date.now() - startedAt}ms cache=${cache.hit ? "hit" : cache.coalesced ? "coalesced" : "miss"}`);
    return json(200, toPublicCancellationsPayload(payload));
  } catch (error) {
    if (error?.code === "config") return json(503, { error: error.message, code: "config" });
    console.error("[Data] cancellations failed:", error instanceof Error ? error.message : error);
    return json(500, {
      error: "Não foi possível consultar cancelamentos.",
