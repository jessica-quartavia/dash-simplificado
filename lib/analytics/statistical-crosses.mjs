...
} from "./correlation-matrix.mjs";
import { buildCohortRetention } from "./cohort-retention.mjs";
import { buildStatisticalDiscoveries } from "./statistical-discoveries.mjs";
import { buildAxisMatricesBundle } from "./sc-axis-matrices.mjs";
import { buildExploratoryBundle } from "./sc-exploratory-ext.mjs";
import { buildClientInsightsBundle } from "./sc-client-insights.mjs";
import {
  buildStatAnalysisContext,
  getChurnNumericSeries,
  getTenureNumericSeries,
  buildNpsClassOthers,
} from "./stat-analysis-context.mjs";
import { createStatComputeProfiler } from "./stat-compute-profiler.mjs";
import { buildChurnAnalysis, buildRenewalAnalysis } from "./univariate-metrics.mjs";
import {
  isSnapshotFresh,
  isStatisticalSnapshotEnabled,
...
function applyPopulationFilters(clients, filters = {}, now = new Date()) {
  ...
  return clients.filter((c) => {
    ...
    if (filters.program && filters.program !== "all" && !programMatches(c, filters.program)) return false;
    ...
    if (filters.status && filters.status !== "all") {
      ...
      if (filters.status === "inactive" && (c.isActive || c.isCancelled || c.isFrozen)) return false;
...
function analyzeNumericVariable(def, active, cancelled, minSample, precomputed = null) {
  const aVals = precomputed?.aVals ?? active.map((c) => c[def.field]).filter((v) => v != null && Number.isFinite(Number(v))).map(Number);
  const cVals = precomputed?.cVals ?? cancelled.map((c) => c[def.field]).filter((v) => v != null && Number.isFinite(Number(v))).map(Number);
  const allForMissing = precomputed?.allForMissing ?? [...active, ...cancelled];
  const miss = precomputed?.miss ?? missingRate(allForMissing, def.field);
  ...
  const xs = precomputed?.xs ? [...precomputed.xs] : [];
  const ys = precomputed?.ys ? [...precomputed.ys] : [];
  if (!precomputed) {
...
export function analyzePopulation(clients, { minSample = MIN_GROUP, includeFrozenSeparate = false, minCoverage = null, analysisContext = null, profileSection = null } = {}) {
  const ctx = analysisContext || buildStatAnalysisContext(clients, { numericVars: NUMERIC_VARS, categoricalVars: CATEGORICAL_VARS });
  const result = buildChurnAnalysis(ctx, {
    numericVars: NUMERIC_VARS,
    categoricalVars: CATEGORICAL_VARS,
    minSample,
    minCoverage,
    includeFrozenSeparate,
    methodologyExcluded: METHODOLOGY_EXCLUDED,
    section: profileSection,
  });
  if (profileSection) profileSection.mergeIntoParent();
  const { univariateMetrics: _registry, ...publicResult } = result;
  return publicResult;
}
...
export function analyzeRenewalAssociations(clients, minSample = MIN_GROUP, analysisContext = null) {
  const ctx = analysisContext || buildStatAnalysisContext(clients, { numericVars: NUMERIC_VARS, categoricalVars: CATEGORICAL_VARS });
  const bundle = buildRenewalAnalysis(ctx, NUMERIC_VARS, CATEGORICAL_VARS, minSample);
  return {
    eligible: bundle.eligible,
    renewed: bundle.renewed,
    notRenewed: bundle.notRenewed,
    sampleSmall: bundle.sampleSmall,
...
export function compareRenewedVsNot(clients, minSample = MIN_GROUP, analysisContext = null) {
  const ctx = analysisContext || buildStatAnalysisContext(clients, { numericVars: NUMERIC_VARS, categoricalVars: CATEGORICAL_VARS });
  return buildRenewalAnalysis(ctx, NUMERIC_VARS, CATEGORICAL_VARS, minSample).compareRows;
}

/** Grupos NPS: promotores / neutros / detratores. */
export function buildNpsGroupsComparison(clients, analysisContext = null) {
  const ctx = analysisContext || buildStatAnalysisContext(clients, { numericVars: NUMERIC_VARS, categoricalVars: CATEGORICAL_VARS });
  const withNps = ctx.withNps;
  const keys = [
  ...
  return keys.map(({ key, label }) => {
    const g = ctx.npsByClass.get(key) || [];
    const others = buildNpsClassOthers(withNps, key);
...
export function analyzeTenureCorrelations(clients, analysisContext = null) {
  const ctx = analysisContext || buildStatAnalysisContext(clients, { numericVars: NUMERIC_VARS, categoricalVars: CATEGORICAL_VARS });
  const pool = ctx.tenurePool;
  const eventCount = pool.filter((c) => c.isCancelled && c.hasConfirmedDate).length;
  const censoredCount = pool.length - eventCount;
  const stayVals = ctx.stayVals;
  const stayMedian = ctx.stayMedian;
  const highStay = ctx.highStay;
  const lowStay = ctx.lowStay;
  const rows = [];
  ...
  for (const def of NUMERIC_VARS) {
    ...
    const pre = getTenureNumericSeries(ctx, def);
    const xs = pre.xs;
    const ys = pre.ys;
    ...
    const valsHigh = pre.valsHigh;
    const valsLow = pre.valsLow;
    let stdDiff = null;
...
export function buildNpsCorrelations(clients, analysisContext = null) {
  const ctx = analysisContext || buildStatAnalysisContext(clients, { numericVars: NUMERIC_VARS, categoricalVars: CATEGORICAL_VARS });
  const pool = ctx.withNps.filter((c) => c.npsPredictiveOk && c.npsScore != null);
  const rows = [];
  for (const def of NUMERIC_VARS) {
...
function parseFiltersFromRequest(request) {
  try {
    ...
    const program = get("program") || "all";
    const programsFromList = (get("programs") || "").split(",").map((value) => value.trim()).filter(Boolean);
    return {
      search: get("search") || "",
      ...
      programs: programsFromList.length ? programsFromList : (program && program !== "all" ? [program] : []),
      program,
    };
  } catch {
...
export async function computeStatisticalCrossesPayload(options = {}) {
  ...
  const now = new Date();
  const profile = createStatComputeProfiler(Boolean(options.perfDebug || options.profileCompute));
  ...
  const accessToken = String(options.accessToken || "").trim() || null;
  const snapshotEnabled = isStatisticalSnapshotEnabled() && !forceLive && Boolean(accessToken);

  if (snapshotEnabled) {
    try {
      const loaded = await statisticalSnapshotStore.loadActiveClientFeatures({ accessToken });
      if (loaded?.clients?.length && isSnapshotFresh(loaded.meta)) {
        dataSource = "snapshot";
        clients = loaded.clients.map((c) => ({ ...c }));
        built = { clients, warnings: [] };
        npsJoin = loaded.meta?.nps_join || {
          joined: clients.filter((c) => c.hasNps).length,
          excludedPostCancel: clients.filter((c) => c.hasNps && !c.npsPredictiveOk).length,
          uniqueNpsClients: clients.filter((c) => c.hasNps).length,
        };
        profile.setFetchMs(Date.now() - fetchStarted);
      } else {
        console.warn("[StatSnapshot] stale or empty, fallback=live");
      }
    } catch (error) {
      const code = error?.code || "unknown";
      if (code === "snapshot_table_missing") {
        console.warn("[StatSnapshot] table missing, fallback=live");
      } else {
        console.warn("[StatSnapshot] unavailable, fallback=live", error instanceof Error ? error.message : error);
      }
    }
  }
  ...
  if (dataSource === "live") {
    ...
    profile.setFetchMs(Date.now() - fetchStarted);
    built = profile.mark("build_client_features", () => buildAnalyticalPopulation(general, meetings, mechanisms, now));
    clients = built.clients;
    npsJoin = profile.mark("nps", () => joinLatestNpsOntoClients(clients, npsRows));
  } else {
    profile.mark("build_client_features", () => null);
    profile.mark("nps", () => null);
  }
  ...
  const analysisContext = buildStatAnalysisContext(clients, { numericVars: NUMERIC_VARS, categoricalVars: CATEGORICAL_VARS });

  const minCoverageFilter = filters.minCoverage != null && Number.isFinite(Number(filters.minCoverage))
    ? Number(filters.minCoverage)
    : null;

  const cancelSection = profile.section("cancelamento");
  const analysis = profile.mark("cancelamento", () =>
    analyzePopulation(clients, {
      minSample: Number(filters.minSample) || MIN_GROUP,
      includeFrozenSeparate: includeFrozen,
      minCoverage: minCoverageFilter,
      analysisContext,
      profileSection: cancelSection,
    }),
  );

  const renewalSection = profile.section("renovacao");
  const renewalBundle = profile.mark("renovacao", () => {
    const renewalCtx = buildStatAnalysisContext(renewalUniverse, {
      numericVars: NUMERIC_VARS,
      categoricalVars: CATEGORICAL_VARS,
    });
    const bundle = buildRenewalAnalysis(
      renewalCtx,
      NUMERIC_VARS,
      CATEGORICAL_VARS,
      Number(filters.minSample) || MIN_GROUP,
      renewalSection,
    );
    renewalSection.mergeIntoParent();
    return bundle;
  });
  const renewalAssociations = {
    eligible: renewalBundle.eligible,
    renewed: renewalBundle.renewed,
    notRenewed: renewalBundle.notRenewed,
    sampleSmall: renewalBundle.sampleSmall,
    numeric: renewalBundle.numeric,
    categorical: renewalBundle.categorical,
  };
  const renewedVsNotRenewed = renewalBundle.compareRows;
  const npsGroups = profile.mark("nps", () => buildNpsGroupsComparison(clients, analysisContext));
  const tenureCorrelations = profile.mark("permanencia", () => analyzeTenureCorrelations(clients, analysisContext));
  const tenureBuckets = profile.mark("permanencia", () => buildTenureBuckets(clients));
  const npsCorrelations = profile.mark("nps", () => buildNpsCorrelations(clients, analysisContext));
  ...
  const npsAnalysis = profile.mark("nps", () =>
    buildNpsAnalysis(clients, null, {
      minSample: Number(filters.minSample) || MIN_GROUP,
    }),
  );
  ...
  const correlationMatrix = profile.mark("general_matrix", () =>
    buildCorrelationMatrix(clients, {
      method: "spearman",
  ...
  const cohort = profile.mark("cohort", () =>
    buildCohortRetention(clients, {
  ...
  const { discoveries, report } = profile.mark("predictive_ranking", () =>
    buildStatisticalDiscoveries({
  ...
  const axisMatrices = profile.mark("group_matrix", () =>
    buildAxisMatricesBundle({
      clients,
      ...
      renewalAssociations,
      renewedVsNotRenewed,
      statAnalysisContext: analysisContext,
    }),
  );
  const exploratory = profile.mark("predictive_ranking", () =>
    buildExploratoryBundle({
      clients,
      associations: analysis.associations || [],
      univariatePredictivePower,
      npsCorrelations,
  ...
  const riskRulesPreview = profile.mark("active_risk", () => buildRiskRulesPreview(clients));
  const clientInsights = profile.mark("top_clients", () =>
    buildClientInsightsBundle({
      clients,
      associations: analysis.associations || [],
  ...
  const payload = profile.mark("serialization", () => ({
  ...
  const serializationMs = profile.snapshot().serialization_ms;
  profile.setSerializationMs(serializationMs);
  const computeProfile = profile.snapshot();
  payload._fetchStats = getAnalyticsDataContext()?.snapshot() || [];
  payload.performance = {
    fetchSources: payload._fetchStats,
    dataSource,
    total_ms: Date.now() - startedAt,
    fetch_ms: computeProfile.fetch_ms,
    compute_ms: computeProfile.compute_ms,
    serialization_ms: computeProfile.serialization_ms,
    computeProfile,
  };
  getAnalyticsDataContext()?.logPerf({
    total_ms: payload.performance.total_ms,
    compute_ms: payload.performance.compute_ms,
    payload_bytes: Buffer.byteLength(JSON.stringify(payload)),
  });
  if (options.writeComputeProfile) {
    const { writeFileSync, mkdirSync } = await import("node:fs");
    const { dirname, join, resolve } = await import("node:path");
    const { fileURLToPath } = await import("node:url");
    const root = resolve(fileURLToPath(new URL("..", import.meta.url)), "..");
    const outPath = join(root, "docs/statistical-compute-profile.json");
    mkdirSync(dirname(outPath), { recursive: true });
    writeFileSync(outPath, JSON.stringify(computeProfile, null, 2));
  }
  return payload;
  }, { perfDebug: perfDebugFromOptions(options), page: "statistical_crosses" });
}

/** Parse filtros a partir de URLSearchParams (V2 dashboard). */
export function parseStatisticalCrossesFilters(searchParams) {
  try {
...
export function toPublicStatisticalCrossesPayload(payload) {
  return payload;
}
