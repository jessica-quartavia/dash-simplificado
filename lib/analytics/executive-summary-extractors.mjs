...
import { filterMechanismClients, expandMechanismSourceRows, filterPortfolio, portfolioSize } from "./mechanism-filters.mjs";
...
import {
  ...
  rankEpRenewedShare,
} from "./ep-executive-ranking.mjs";
import {
  executiveSignalDisplayLabel,
  executiveSignalDistributionLabel,
} from "./executive-signal-labels.mjs";
...
export function buildExecutiveContexts(sources = {}, filters = {}) {
  ...
  try {
    if (sources.general?.clients) {
      ...
      const acquisitionRows = filterGeneralAcquisitionRows(sources.general.clients, gf, { ignoreStatus: true });
  ...
  try {
    if (sources.mechanisms?.clients) {
      const domainFilters = executiveFiltersToDomain(filters, "mechanisms");
      const rows = filterMechanismClients(sources.mechanisms.clients, domainFilters);
      const sourceRows = expandMechanismSourceRows(rows, sources.mechanisms.clients, domainFilters);
      const portfolio = filterPortfolio(sources.mechanisms.portfolio || [], domainFilters);
      const meta = sources.mechanisms.metadata || {};
  ...
  try {
    if (sources.satisfaction?.clients || sources.satisfaction?.scopeInputs) {
      ...
      const csatSummary = summarizeSatisfactionRows(rows, populationTotal);
      const npsKpi = resolveSatisfactionNpsKpi(sources.satisfaction, scoped);
      contexts.satisfaction = {
        rows,
        summary: { ...csatSummary, ...npsKpi },
        npsKpi,
        dist: distributionsFromSatisfactionRows(rows, sources.satisfaction.distributions || {}, npsKpi),
...
export function extractExecutiveMetrics(contexts = {}) {
  ...
  const clientsWithImplemented = mech?.summary?.displayedImplementedClients
    ?? mech?.summary?.clientsWithImplementedMechanism ?? null;
  const mechPopulation = mech?.summary?.displayedClients
    ?? mech?.summary?.displayedCombinedTotal ?? mech?.summary?.clientsWithMechanisms ?? null;
  const mechSourceNote = mech?.summary
    ? `BASE QV ${mech.summary.baseQvDisplayed ?? 0} · App Pharus ${mech.summary.appPharusDisplayed ?? 0}`
    : null;
  const mechImplementationRate = mech?.summary?.displayedImplementationRate
    ?? mech?.summary?.mechanismImplementationRate ?? mech?.summary?.implementationPercent ?? null;
  const clientsWithImpl = mech?.summary?.displayedImplementedClients
    ?? mech?.summary?.clientsWithImplementedMechanism ?? null;
  const clientsWithLinked = mech?.summary?.displayedClients
    ?? mech?.summary?.clientsWithLinkedMechanisms ?? mech?.summary?.clientsWithMechanismsRows ?? null;
  const mechDist = topMechanismDistribution(mech?.summary?.typeUsage || [], 5);

  const epRenewed = ep?.engineers ? rankEpRenewedShare(ep.engineers) : null;
  const epImpl = ep?.engineers ? rankEpImplementationShare(ep.engineers) : null;
  ...
  const temporalSignals = (ti?.preCancellation?.signals || [])
    ...
    .map((s) => ({
      key: s.key || s.id || null,
      label: executiveSignalDisplayLabel(s),
      count: s.count,
      percent: s.percent,
  ...
  const temporalDistribution = (
    ...
    ...item,
    label: executiveSignalDistributionLabel(item.label),
  }));
  ...
  return {
    ...
    clients_with_implemented_mechanisms: mech
      ? metricValue(clientsWithImplemented, {
          numerator: clientsWithImpl,
          denominator: clientsWithLinked,
          coverage: { denominator: clientsWithLinked },
          note: mechSourceNote,
        })
      : null,
    clients_implementation_rate: mech
      ? metricValue(mechImplementationRate, {
          rule: "clientes com ≥1 mecanismo implantado ÷ clientes com ≥1 mecanismo vinculado",
          numerator: clientsWithImpl,
          denominator: clientsWithLinked,
          note: mechSourceNote
            ? `${mechSourceNote} · mesma fórmula da página Implementação de Mecanismos.`
            : "Mesma fórmula da página Implementação de Mecanismos (% com mecanismo implantado).",
        })
      : null,
    mechanism_type_distribution: mech ? metricValue(mechDist) : null,

    nps: sat
      ? metricValue(sat.summary?.nps ?? null, {
          coverage: {
            responses: sat.summary?.npsResponses ?? 0,
            percent: sat.summary?.npsCoveragePercent,
            denominator: sat.rows?.length ?? 0,
          },
        })
      : null,
    ...
    intention_destination_branches: can
      ...
      : null,
    renewed_active_clients_rate: ren
      ? metricValue(ren.summary?.renewedActiveRate ?? null, {
          numerator: ren.summary?.renewedActiveClients ?? null,
          denominator: ren.summary?.activeClients ?? null,
          rule: "clientes ativos com ciclo ≥ 2 ÷ clientes analiticamente ativos",
        })
      : null,

    top_ep_renewed_share: epRenewed
      ? metricValue({
          high: epRenewed.high,
          low: epRenewed.low,
          minSample: epRenewed.minSample,
        })
      : null,
    top_ep_implementation_share: epImpl
      ? metricValue({
          high: epImpl.high,
          low: epImpl.low,
          minSample: epImpl.minSample,
        })
      : null,

    temporal_top_signals: ti ? metricValue(temporalSignals) : null,
    temporal_signal_distribution: ti ? metricValue(temporalDistribution) : null,
  };
}
...
export function compareExecutiveMetrics(executiveMetrics = {}, sourceMetrics = {}) {
  const rows = EXECUTIVE_METRIC_REGISTRY.map((def) => {
    ...
    return {
      ...
      executiveValue,
      sourceValue,
      match: executiveValue === sourceValue,
      pending: false,
    };
  });
  return {
...
export function buildOfficialSourceMetrics(contexts = {}) {
  ...
  const mechPopulation = mech?.summary?.displayedCombinedTotal ?? mech?.summary?.clientsWithMechanisms ?? null;
  const clientsWithImpl = mech?.summary?.clientsWithImplementedMechanism ?? null;
  const clientsWithLinked = mech?.summary?.clientsWithLinkedMechanisms ?? mech?.summary?.clientsWithMechanismsRows ?? null;
  ...
  return {
    ...
    clients_implementation_rate:
      mech?.summary?.mechanismImplementationRate ?? mech?.summary?.implementationPercent ?? (clientsWithLinked ? pct(clientsWithImpl, clientsWithLinked) : null),
    ...
    intention_destination_branches: contexts.cancellations
      ...
      : null,
    renewed_active_clients_rate: contexts.renewal?.summary?.renewedActiveRate ?? null,
    top_ep_renewed_share: (() => {
      const ranked = ep?.engineers ? rankEpRenewedShare(ep.engineers) : null;
      return ranked ? { high: ranked.high, low: ranked.low, minSample: ranked.minSample } : null;
    })(),
    top_ep_implementation_share: (() => {
      const ranked = ep?.engineers ? rankEpImplementationShare(ep.engineers) : null;
      return ranked ? { high: ranked.high, low: ranked.low, minSample: ranked.minSample } : null;
    })(),
    temporal_top_signals: executive.temporal_top_signals?.value ?? null,
    temporal_signal_distribution: executive.temporal_signal_distribution?.value ?? null,
  };
}
