...
import { filterSatisfactionClients } from "./satisfaction-filters.mjs";
...
function signalCountDistribution(clients = []) {
  ...
  for (const row of clients) {
    ...
    if (count <= 0) buckets.set("0 sinais", buckets.get("0 sinais") + 1);
    else if (count === 1) buckets.set("1 sinal", buckets.get("1 sinal") + 1);
    else if (count === 2) buckets.set("2 sinais", buckets.get("2 sinais") + 1);
    else if (count === 3) buckets.set("3 sinais", buckets.get("3 sinais") + 1);
    else buckets.set("4+ sinais", buckets.get("4+ sinais") + 1);
  }
...
export function buildExecutiveContexts(sources = {}, filters = {}) {
  ...
  try {
    if (sources.satisfaction?.clients) {
      const rows = filterSatisfactionClients(
        sources.satisfaction.clients,
        executiveFiltersToDomain(filters, "satisfaction"),
      );
      contexts.satisfaction = {
        rows,
        summary: summarizeSatisfactionRows(rows),
...
export function extractExecutiveMetrics(contexts = {}) {
  ...
  return {
    ...
    top_cancellation_reasons: can
      ? metricValue(topCategorizedCancellationReasons(can.dist.byCategory || [], 3), {
          note: "Principais motivos categorizados. Motivos não categorizados foram excluídos desta leitura.",
        })
      : null,
    ...
    renewal_eligible_clients: metricValue(null, {
      status: "pending_rule",
      pending: RENEWAL_ELIGIBLE_RULE_STATUS,
    }),
    ...
    temporal_signal_distribution: ti
      ? metricValue(signalCountDistribution(ti.preCancellation?.clients || []))
      : null,
  };
}
...
export function compareExecutiveMetrics(executiveMetrics = {}, sourceMetrics = {}) {
  const rows = EXECUTIVE_METRIC_REGISTRY.map((def) => {
    ...
    return {
      ...
      executiveValue,
      sourceValue,
      match: def.id === "renewal_eligible_clients" ? executive?.status === "pending_rule" : match,
      pending: def.id === "renewal_eligible_clients",
    };
  });
  return {
...
export function buildOfficialSourceMetrics(contexts = {}) {
  ...
  return {
    ...
    top_cancellation_reasons: contexts.cancellations
      ? topCategorizedCancellationReasons(contexts.cancellations.dist?.byCategory || [], 3)
      : null,
    ...
    renewal_eligible_clients: null,
    renewals_per_active_client: executive.renewals_per_active_client?.value ?? null,
...
export { signalCountDistribution };
