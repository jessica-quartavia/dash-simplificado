...
import { filterMechanismClients, filterPortfolio, portfolioSize } from "./mechanism-filters.mjs";
...
export function extractExecutiveMetrics(contexts = {}) {
  ...
  const mechImplementationRate = mech?.summary?.mechanismImplementationRate ?? mech?.summary?.implementationPercent ?? null;
  ...
  return {
    ...
    clients_implementation_rate: mech
      ? metricValue(mechImplementationRate, {
          ...
          numerator: clientsWithImpl,
          denominator: clientsWithLinked,
          note: "Mesma fórmula da página Implementação de Mecanismos (% com mecanismo implantado).",
        })
      : null,
