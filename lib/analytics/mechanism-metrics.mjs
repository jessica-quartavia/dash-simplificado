...
export function summarizeMechanismRows(
  ...
  const baseQvDisplayed = clientsWithMechanismsRows;
  const appPharusDisplayed = pharusAvailable
    ? (clientAudit.pharusUsersWithMechanisms ?? clientAudit.appPharus ?? 0)
    : 0;
  ...
  const clientsWithLinkedMechanisms = clientsWithMechanismsRows;
  ...
  const typeUsage = sortUnknownLast(
    [...typeMap.values()].map((item) => {
      const linkedCount = item.clients.size;
      const implementedCount = item.implementedClients.size;
      return {
        label: item.label,
        count: linkedCount,
        linkedCount,
