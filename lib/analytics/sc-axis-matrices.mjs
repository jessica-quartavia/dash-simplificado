...
export function buildTargetCorrelationMatrix(clients, {
  ...
  minN = 30,
  stdDiffGroups = null,
  statAnalysisContext = null,
} = {}) {
  const total = clients.length;
  let groupA = [];
  let groupB = [];
  if (stdDiffGroups?.groupA && stdDiffGroups?.groupB) {
    if (statAnalysisContext?.groups?.promoter && statAnalysisContext?.groups?.detractor && targetField === "npsScore") {
      groupA = statAnalysisContext.groups.promoter.filter((c) => c.npsPredictiveOk && c.npsScore != null);
      groupB = statAnalysisContext.groups.detractor.filter((c) => c.npsPredictiveOk && c.npsScore != null);
    } else if (statAnalysisContext?.highStay?.length && statAnalysisContext?.lowStay?.length && targetField === "stayDays") {
      groupA = statAnalysisContext.highStay;
      groupB = statAnalysisContext.lowStay;
    } else {
      groupA = clients.filter(stdDiffGroups.groupA);
      groupB = clients.filter(stdDiffGroups.groupB);
    }
  }
