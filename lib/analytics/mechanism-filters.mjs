...
/** Expande recorte source_sum com usuários App-only quando Programa=Todos. */
export function expandMechanismSourceRows(displayRows, allClients, filters = {}) {
  const display = Array.isArray(displayRows) ? displayRows : [];
  const all = Array.isArray(allClients) ? allClients : [];
  const f = { ...defaultMechanismFilters(), ...filters };
  const statusFilter = normalizeStatusFilter(f.status);
  const displayIds = new Set(display.map((row) => String(row.clientId || "")));
  const extras = [];

  for (const row of all) {
    const id = String(row.clientId || "");
    if (!id.startsWith("pharus:") || displayIds.has(id)) continue;
    if (!rowHasLinkedMechanism(row)) continue;
    if (!programMatches(row, f.program)) continue;
    if (matchesAnalyticalStatusFilter(row.analyticalStatus, statusFilter)) {
      extras.push(row);
      continue;
    }
    if (f.program === "all") extras.push(row);
  }
