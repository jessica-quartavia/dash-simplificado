...
export async function handleInternalMechanismsSatisfactionRequest(request, deps = {}) {
  ...
  try {
    ...
    const rp = publicPayload.renewalYearEndProjection;
    console.info(
      `[IMS] renewalProjection present=${Boolean(rp)} error=${Boolean(rp?.error)} proxyAvailable=${Boolean(rp?.proxyAvailable ?? rp?.proxyOperational)} projectionAvailable=${Boolean(rp?.projectionAvailable)} horizon.total=${rp?.metrics?.horizon?.total ?? "—"} cache=${cache.status} key=${cache.key ?? "—"} force=${force ? "1" : "0"}`,
    );
    ...
    return json(200, {
      ...publicPayload,
      meta: {
        calculationVersion: publicPayload.calculationVersion,
        serverCalculationVersion: CALCULATION_VERSION,
        renewalProjectionPresent: Boolean(rp),
        renewalProjectionError: Boolean(rp?.error),
        renewalProjectionProxyAvailable: Boolean(rp?.proxyAvailable ?? rp?.proxyOperational),
        renewalProjectionAvailable: Boolean(rp?.projectionAvailable),
      },
    });
  } catch (error) {
