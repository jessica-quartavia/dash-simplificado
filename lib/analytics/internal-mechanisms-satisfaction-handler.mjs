...
export async function handleInternalMechanismsSatisfactionRequest(request, deps = {}) {
  ...
  try {
    ...
    console.info(
      `[IMS] compute complete clientsWithNps=${payload?.summary?.clientsWithNps ?? "—"} cache=${cache.status} key=${cache.key ?? "—"} force=${force ? "1" : "0"}`,
    );
    const responseBytes = JSON.stringify(publicPayload).length;
    const totalMs = Date.now() - startedAt;
    console.info(
      `[IMS] response_bytes=${responseBytes} total_ms=${totalMs}ms cache=${cache.status}`,
    );
    console.info(
