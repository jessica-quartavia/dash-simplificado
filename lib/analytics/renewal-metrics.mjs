...
export function summarizeRenewalActiveClients(rows) {
  const list = Array.isArray(rows) ? rows : [];
  const active = list.filter((r) => r.analyticalStatus === "Ativo");
  const renewedActive = active.filter((r) => r.cycleValid && r.renewed);
  return {
    activeClients: active.length,
    renewedActiveClients: renewedActive.length,
    renewedActiveRate: pct(renewedActive.length, active.length),
  };
}
...
export function summarizeRenewalRows(rows) {
  ...
  const activeRenewal = summarizeRenewalActiveClients(list);
