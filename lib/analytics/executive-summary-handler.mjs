...
import { computeWithPageCache } from "./handler-cache.mjs";
...
export async function handleExecutiveSummaryRequest(request, deps = {}) {
  ...
  try {
    ...
    const { payload, cache } = await computeWithPageCache(
      "executive_summary",
      () => compute(filters, deps, { force, perfDebug: perfDebugFromRequest(request) }),
    ...
    console.info(
      `[executive-summary] status=200 total=${Date.now() - startedAt}ms domains=${Object.keys(payload.performance?.domains || {}).length} cache=${cache.hit ? "hit" : cache.coalesced ? "coalesced" : "miss"}`,
    );
    if (typeof process !== "undefined" && (process.env.NODE_ENV !== "production" || process.env.EXECUTIVE_DEBUG === "1")) {
      console.info("[Executive] build payload");
    }
