 ...
 */
import { requireCorporateAuth, getRequestAccessToken } from "../auth.mjs";
...
export async function handleStatisticalCrossesRequest(request, deps = {}) {
  ...
  try {
    ...
    const { payload, cache } = await computeWithPageCache(
      "statistical_crosses",
      () => compute({ filters, perfDebug: perfDebugFromRequest(request), accessToken }),
      { request, filters, force },
    );
    if (method === "HEAD") {
      return new Response(null, { status: 200, headers: { "Cache-Control": "no-store" } });
    }
    console.info(`[statistical-crosses] status=200 total=${Date.now() - startedAt}ms cache=${cache.status}`);
