/**
 * Payload de Implementação de Mecanismos — BASE QV somente leitura.
 * Indicadores CSV desta página usam public.clients + public.client_mecanismos + public.mecanismos.
 * App Pharus é seção à parte na V1 e não consta no Dash Kids desta página.
 */
...
const CLIENT_SELECT =
  "id,codigo,name,status,engenheiro_patrimonial,data_inicio_ciclo,created_at,data_churn,email,programa";
const CM_SELECT =
...
export function buildMechanismsPayload({
  ...
  return {
    ...
    metadata: {
      ...
      pharusNote: "App Pharus não entra nesta página: o CSV Dash Kids usa somente BASE QV.",
    },
  };
}
...
export async function computeMechanismsPayload() {
  ...
  const payload = buildMechanismsPayload({
    clients,
    cmRows: (cmTimed.value || []).filter((row) => !removedIds.has(String(row.client_id || ""))),
