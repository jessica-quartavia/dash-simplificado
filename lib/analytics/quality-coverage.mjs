 ...
 * @param {{ optionalTables?: Set<string>, rowsByTable?: Map<string, object[]>, loadErrors?: Map<string, Error> }} opts
 */
export async function scanFieldCoverage(fields, { optionalTables = new Set(), rowsByTable: preloaded, loadErrors: preloadedErrors } = {}) {
  const tableMap = buildTableColumnMap(fields);
  const rowsByTable = preloaded ? new Map(preloaded) : new Map();
  const loadErrors = preloadedErrors ? new Map(preloadedErrors) : new Map();
  ...
  if (!preloaded) {
    ...
    for (const table of tableMap.keys()) {
      if (!rowsByTable.has(table) && !loadErrors.has(table)) {
        rowsByTable.set(table, []);
      }
    }
  }
...
/** Pré-carrega todas as tabelas da Qualidade via AnalyticsDataContext (uma Promise por tabela). */
export async function preloadQualityFieldRows(fields, { optionalTables = new Set() } = {}) {
  const tableMap = buildTableColumnMap(fields);
