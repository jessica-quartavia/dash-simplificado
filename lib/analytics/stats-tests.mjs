...
/** Ranks de Spearman para vetor numérico (empates = rank médio). Exportado para cache por variável. */
export function computeAverageRanks(values) {
  return averageRanks(values);
}

/**
 * Spearman a partir de ranks já calculados sobre o mesmo subconjunto pareado.
 * @param {number[]} rankX
 * @param {number[]} rankY
 */
export function spearmanFromRanks(rankX, rankY) {
  const n = Math.min(rankX?.length || 0, rankY?.length || 0);
  if (n < 3) {
...
export function logisticUnivariateAuc(x, y, { folds = 5, seed = 42 } = {}) {
  ...
  const trainParts = Array.from({ length: k }, () => []);
  for (let f = 0; f < k; f += 1) {
    for (let j = 0; j < k; j += 1) {
      if (j === f) continue;
      trainParts[f].push(...foldsData[j]);
    }
  }
  const aucs = [];
  for (let f = 0; f < k; f += 1) {
    const train = trainParts[f];
    const test = foldsData[f];
    const m = fitLogistic(train);
    if (!m) continue;
    const scores = new Array(test.length);
    const labels = new Array(test.length);
    for (let t = 0; t < test.length; t += 1) {
      scores[t] = sigmoid(m.b0 + m.b1 * test[t].x);
      labels[t] = test[t].y;
    }
...
export function buildContingencyFromGroups(activeLabels, cancelledLabels) {
  ...
  for (const lab of activeLabels) {
    if (lab == null || lab === "" || seen.has(lab)) continue;
    seen.add(lab);
    labels.push(lab);
  }
  for (const lab of cancelledLabels) {
    if (lab == null || lab === "" || seen.has(lab)) continue;
    seen.add(lab);
    labels.push(lab);
  }
  ...
  for (const lab of activeLabels) {
    ...
    counts0.set(lab, (counts0.get(lab) || 0) + 1);
  }
  for (const lab of cancelledLabels) {
    if (lab == null || lab === "") continue;
    counts1.set(lab, (counts1.get(lab) || 0) + 1);
  }
  const table = [
    labels.map((lab) => counts0.get(lab) || 0),
    labels.map((lab) => counts1.get(lab) || 0),
  ];
  return { table, labels };
}
