...
import { robustStats } from "./general-metrics.mjs";
import {
...
export async function computeEpPerformancePayload(options = {}) {
  ...
  const [mechanismsPayload, cycleAuditRaw, cycleMap] = await Promise.all([
    computeMechanismsCore(bundle).catch((error) => {
  ...
  const generalPayloadRaw = computeGeneralCore(bundle);
  const meetingsPayload = computeMeetingsCore(bundle);
  const npsRows = bundle.npsRows || [];
  const csatRows = bundle.csatRows || [];
  ...
  return payload;
  }, { perfDebug: perfDebugFromOptions(options), page: "ep_performance" });
}

export function toPublicEpPerformancePayload(payload) {
  ...
  return {
    ...rest,
    engineers: slimEpEngineers(payload.engineers),
    detailsEndpoint: "/api/ep-performance/details",
  };
}

const EP_CLIENT_PUBLIC_FIELDS = [
  "segment",
  "program",
  "analyticalStatus",
  "cancelled",
  "renewed",
  "totalMeetings",
  "hasImplementedMechanism",
  "npsScore",
];

function slimEpClient(client) {
  if (!client || typeof client !== "object") return client;
  const slim = {};
  for (const key of EP_CLIENT_PUBLIC_FIELDS) {
    if (client[key] !== undefined) slim[key] = client[key];
  }
  return slim;
}

function slimEpEngineers(engineers) {
  return (Array.isArray(engineers) ? engineers : []).map((row) => ({
    ...row,
    clients: (row.clients || []).map(slimEpClient),
  }));
}

export function extractEpPerformanceDetails(payload, section = "renewalClients") {
  if (!payload) return {};
  if (section === "renewalClients") {
    return { renewalClients: payload.renewalClients || [] };
  }
  if (section === "mechanismsMatrix") {
    return { mechanismsMatrix: payload.mechanismsMatrix || [] };
  }
  return { error: "Seção desconhecida.", code: "unknown_section" };
}
