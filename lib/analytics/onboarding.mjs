...
export function buildOnboardingPayload({
  ...
  const withCompletionBase = rows.filter((row) => row.completedOnboarding != null).length;
  ...
  return {
    ...
    summary: {
      ...
      completedPercent: withCompletionBase ? Math.round((completeCount / withCompletionBase) * 1000) / 10 : 0,
    },
    clients: rows,
    metadata: {
      completionRule:
