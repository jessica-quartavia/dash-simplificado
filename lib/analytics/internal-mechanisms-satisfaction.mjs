...
export function buildInternalMechanismsSatisfactionPayload(dataset, options = {}) {
  ...
  const renewalYearEndProjection = safeAssessRenewalYearEndProjection(npsPopulation, {
    portfolioClients: allWide,
    catalog,
    minMechanismSample: minMechN,
  });
  if (process.env.NODE_ENV !== "production") {
    const rp = renewalYearEndProjection;
    console.info(
      `[IMS] compute renewalProjection exists=${Boolean(rp)} error=${Boolean(rp?.error)} horizon.total=${rp?.metrics?.horizon?.total ?? "—"} modelPublished=${Boolean(rp?.modelProjectionPublished)} proxy=${rp?.proxyClassification ?? "—"}`,
    );
  }
  ...
  return {
    ...
    calculationVersion: IMS_RENEWAL_PROJECTION_VERSION,
    experimental: true,
