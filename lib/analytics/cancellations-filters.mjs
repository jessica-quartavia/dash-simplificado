...
export function normalizeArchivedFilter(value) {
  const raw = String(value || "no").trim();
  if (raw === "yes" || raw === "no" || raw === "all") return raw;
  return "no";
}

export function matchesArchived(row, archivedFilter) {
  const filter = normalizeArchivedFilter(archivedFilter);
  if (filter === "all") return true;
  if (filter === "yes") return Boolean(row?.isArchived);
...
export function defaultCancellationFilters() {
  return {
    ...
    reasonCategory: "all",
    responsible: "all",
    /** V1 default: ocultar clientes com registro arquivado nos KPIs e tabela. */
    archived: "no",
    ...defaultPeriodState(),
  };
}
...
export function filterCancellationClients(clients, filters = {}, options = {}) {
  ...
  return (Array.isArray(clients) ? clients : []).filter((row) => {
    ...
    if (!matchesArchived(row, f.archived)) return false;
