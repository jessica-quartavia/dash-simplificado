...
async function handleDelete(reportId, accessToken, store, requestId) {
  ...
  try {
    ...
    if (storageMissing) {
      console.warn(`[reports] delete storage missing id=${id} request=${requestId} path=${row.storage_path}`);
    }
  } catch (error) {
    if (error?.code === "storage_delete_failed") {
      console.error(
        `[reports] delete storage failed id=${id} request=${requestId} status=${error.status} postgrest=${error.postgrestCode || "n/a"} code=${error.code}`,
      );
      return deleteErrorResponse(error.status === 403 ? 403 : 500, {
        ...
        code: error.code,
        hint: "Aplique sql/analytics/010_reports_delete_corporate.sql no Business Data (tabela + Storage).",
      }, requestId, error);
    }
