...
function mapStoreError(error) {
  ...
  if (code === "reports_rpc_missing") {
    return json(503, {
      error: reportsErrorMessage(code),
      code,
      hint: "Aplique sql/analytics/016_reports_public_rpc.sql no Business Data.",
    });
  }
  if (code === "reports_schema_not_exposed") {
    return json(503, {
      ...
      hint: "Inclua analytics em Settings → API → Exposed schemas ou use RPC 016.",
    });
  }
...
async function handleList(accessToken, store) {
  console.info("[Reports] db start");
  const rows = await store.list({ accessToken });
  console.info(`[Reports] db end rows=${Array.isArray(rows) ? rows.length : 0}`);
...
export async function handleReportsRequest(request, deps = {}) {
  ...
  if (deps.requirePageAccess !== false) {
    ...
    const accessLabel = accessView?.isOwner ? "owner" : (accessView?.groups?.includes("product") ? "product" : "other");
    console.info(`[Reports] access=${accessLabel}`);
