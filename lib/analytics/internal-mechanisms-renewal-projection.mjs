...
export const IMS_RENEWAL_PROJECTION_VERSION = "2026-09-24-ims-renewal-proxy-v1";
 ...
 */
export function assessRenewalYearEndProjection(npsPopulation, options = {}) {
  const { today, horizonEnd, year } = yearEndHorizonDate(options.refDate || new Date());
  const portfolio = options.portfolioClients || npsPopulation || [];
  ...
  const activeNps = (npsPopulation || []).filter((c) =>
    matchesAnalyticalStatusFilter(c.analyticalStatus, "active"),
  );
