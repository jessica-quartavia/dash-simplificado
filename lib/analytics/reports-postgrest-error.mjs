...
export function classifyReportsPostgrestError(status, bodyText) {
  ...
  if (
    pg.code === "PGRST202"
    || /function .* does not exist/i.test(haystack)
  ) {
    return {
      code: "reports_rpc_missing",
...
export function reportsErrorMessage(code) {
  switch (code) {
    case "reports_rpc_missing":
      return "Funções de relatórios ainda não foram instaladas no Business Data. Aplique sql/analytics/016_reports_public_rpc.sql.";
    case "reports_schema_not_exposed":
      return "O schema analytics não está exposto na API (PGRST106). Peça para incluir analytics em Exposed schemas no projeto Business Data.";
