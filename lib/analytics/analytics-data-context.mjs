 ...
 * AsyncLocalStorage é só Node. Import estático de `node:async_hooks` quebra o browser
 * (Pesquisa de Satisfação importa este módulo via satisfaction-filters).
 */
import { fetchAllRowsImpl, resolveRestPageSize, resolveRestRangeSize } from "../data/supabase-rest.mjs";
...
export function createFallbackAls() {
  let store;
  return {
    run(nextStore, fn) {
      const prev = store;
...
export async function createAnalyticsAls() {
  if (!isNodeRuntime()) return createFallbackAls();
...
const CANONICAL_SELECT = {
  ...
  nps_responses:
    "id,typeform_response_id,client_id,tipo_de_forms,score,created_at,submitted_at",
  csat_responses:
...
function normalizeSelectKey(select) {
...
function normalizeFetchOpts(opts = {}) {
  if (opts.canonical === false) return opts;
...
function fetchKey(opts = {}) {
  ...
  return JSON.stringify({
    ...
    select: normalizeSelectKey(normalized.select),
...
export function createAnalyticsDataContext(options = {}) {
  ...
  async function trackedFetch(opts) {
    ...
    const normalized = normalizeFetchOpts(opts);
    const promise = fetchAllRowsImpl(normalized)
      .then((rows) => {
        stat.fetch_ms = Date.now() - started;
        stat.total_rows = rows.length;
        const pageSize = resolveRestRangeSize(normalized.table, normalized.pageSize);
...
export function perfDebugFromOptions(options = {}) {
  ...
  if (typeof process !== "undefined" && process.env?.ANALYTICS_PERF_DEBUG === "1") return true;
  return false;
}
