...
import {
  ...
  summarizeClientDisplayKinds,
} from "./support-client-display.mjs";

export const PRIORITY_ORDER = ["Urgente", "Alta", "Média", "Baixa", "Não informado"];
...
export function buildSupportAnalyticsPayload(input = {}) {
  ...
  const testFilter = filterOutTestAcionamentos(acionamentos);
  acionamentos = testFilter.rows;
  ...
  for (const row of acionamentos) {
    ...
    const hasClient = hasSecureBaseQvMatch(tratado, clients) && countClients > 0;
    ...
    const clientDisplayInfo = resolveSupportClientDisplay({
      row,
      tratado,
      clients,
      hasBaseQvMatch: hasClient,
      requester: requesterNorm.label,
      countClients,
    });
    const clientLabel = clientDisplayInfo.display;
    ...
    tickets.push({
      ...
      clientLabel,
      clientDisplay: clientDisplayInfo.display,
      clientDisplayKind: clientDisplayInfo.displayKind,
      clientDisplaySourceField: clientDisplayInfo.sourceField,
      clients,
      ...
      clientName: hasClient
        ? (blankToNull(tratado?.baseqv_client_name) || clientLabel)
        : clientLabel,
  ...
  const clientDisplayAudit = summarizeClientDisplayKinds(tickets);
  ...
  return {
    ...
    meta: {
      ...
      excludedTestTitles: testFilter.excludedCount,
      clientDisplayAudit,
      rawStatus: byStatus,
    },
    tickets,
