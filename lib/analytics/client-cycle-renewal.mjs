...
/** Bucket de quantidade de renovações (max(ciclo−1, 0)) — alinhado ao gráfico V1. */
export function renewalCountDistributionLabel(count) {
  if (count == null || !Number.isFinite(count)) return "Sem dado";
  if (count >= 4) return "4+";
  return String(Math.trunc(Math.max(0, count)));
}

export const RENEWAL_COUNT_BAND_ORDER = ["0", "1", "2", "3", "4+", "Sem dado"];
