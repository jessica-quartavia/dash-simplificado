...
export async function handleSupportRequest(request, deps = {}) {
  ...
  try {
    ...
    const { payload, cache } = await computeWithPageCache(
      "support",
      () => compute({ accessToken, allowN8nFallback: true }),
      { request, force },
    );
    ...
    console.info(`[support] status=200 total=${Date.now() - startedAt}ms cache=${cache.status} tickets=${payload?.summary?.totalTickets ?? 0}`);
