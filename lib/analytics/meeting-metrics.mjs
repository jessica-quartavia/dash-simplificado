...
/** Cliente com reunião válida no recorte — mesma regra de calculateMeetingSummary (V1). */
export function clientHasMeeting(client) {
  if (client?.hasValidMeeting === true) return true;
  if ((client?.journeyMeetingsCount || 0) > 0) return true;
  const ms = client?.meetings || [];
  return ms.some(
    (m) =>
      m.meetingDateStatus === "valid" ||
...
export function summarizeMeetingRows(rows, { periodActive = false, periodDivisor = null, now = new Date() } = {}) {
  ...
  const totalMeetings = rows.reduce((sum, client) => sum + (Number(client.totalMeetings) || 0), 0);
  ...
  const clientsWithMeeting = rows.filter(clientHasMeeting).length;
