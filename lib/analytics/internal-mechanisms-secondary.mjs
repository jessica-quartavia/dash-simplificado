...
export function buildRenewalMechanismRanking(npsPopulation, catalog, minSample = IMS_MECHANISM_MIN_N) {
  const baseRate = buildRenewalComVsSem(
    npsPopulation.filter((c) => (c.totalImplementedMechanisms || 0) > 0),
    npsPopulation.filter((c) => !(c.totalImplementedMechanisms > 0)),
  ).withoutMechanism.ratePct;
  return catalog
    .map((mech) => {
      const field = `implemented_${mech.slug}`;
      ...
      return {
        ...
        renewed,
        renewalRatePct: rate,
        diffVsWithoutMechanismPct:
          rate != null && baseRate != null ? round1(rate - baseRate) : null,
        coveragePct: coveragePct(clients.length, npsPopulation.length),
