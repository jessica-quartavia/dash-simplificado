/**
 * Camada única de leitura REST do App Pharus (anon key only).
 * Nunca usa JWT do Auth/Business Data nem service role.
 */
...
export function getPharusRestConfig() {
  ...
  if (!anonKey) return { ok: false, error: "Configure PHARUS_SUPABASE_ANON_KEY." };
  if (/service_role/i.test(anonKey)) {
    return { ok: false, error: "PHARUS_SUPABASE_ANON_KEY não pode ser service role." };
  }
  return { ok: true, url: trimUrl(url), anonKey };
}
...
function buildHeaders(anonKey, schema) {
  return {
    apikey: anonKey,
...
export async function pharusRestFetch(
  ...
  const headers = buildHeaders(cfg.anonKey, schema);
  if (head) {
    headers.Range = "0-0";
...
/** Compatibilidade com código legado que usa getPharusSupabaseClient(). */
export function createPharusRestClient(options = {}) {
  const schema = (options.schema || getPharusEnv().schema || "core").trim() || "core";
  return {
    schema,
