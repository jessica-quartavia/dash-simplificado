...
import { createPharusRestClient } from "./data/pharus-rest.mjs";
...
/** App Pharus — mecanismos e eventos (backend read-only; somente anon key). */
export function getPharusEnv() {
  ...
  const serviceRoleKey = trimEnv(
    process.env.PHARUS_SUPABASE_SERVICE_ROLE_KEY || process.env.APP_PHARUS_SUPABASE_SERVICE_ROLE_KEY,
  );
  return {
    url,
    anonKey,
    serviceRoleKey: serviceRoleKey || null,
    /** Chave REST server-side — fidelidade V1 com anon: apikey + Authorization Bearer. */
    restKey: anonKey,
    schema,
...
export function pharusConfigurationError() {
  ...
  if (!anonKey) return "Configure PHARUS_SUPABASE_ANON_KEY.";
  if (/service_role/i.test(anonKey)) {
    return "PHARUS_SUPABASE_ANON_KEY não pode ser service role.";
  }
  return httpsError(url, "PHARUS_SUPABASE_URL");
}

/**
 * Cliente REST exclusivo do App Pharus (anon key, read-only).
 * Delega para lib/data/pharus-rest.mjs.
 */
export function getPharusSupabaseClient(options = {}) {
  ...
  return createPharusRestClient(options);
}
